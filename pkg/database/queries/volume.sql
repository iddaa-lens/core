...
UPDATE
    events
SET
    betting_volume_percentage = sqlc.arg(betting_volume_percentage) :: float8,
    volume_rank = sqlc.arg(volume_rank) :: float8,
...
WHERE
    e.betting_volume_percentage >= sqlc.arg(min_volume) :: float8
...
GROUP BY
    ...
    e.volume_rank
HAVING
    COALESCE(MAX(ABS(co.movement_percentage)), 0) >= sqlc.arg(min_movement) :: float8
ORDER BY
...
FROM
    ...
    LEFT JOIN current_odds co ON co.event_id = e.id
WHERE
    e.betting_volume_percentage <= sqlc.arg(max_volume) :: float8
...
WHERE
    bvh.event_id = sqlc.arg(event_id)
ORDER BY
...
-- name: BulkInsertVolumeHistory :execrows
...
-- name: GetEventIDsByExternalIDs :many
...
FROM
    events
WHERE
    external_id = ANY(sqlc.arg(external_ids) :: text []);