...
-- name: GetAllActiveEventsForDetailedSync :many
SELECT
  *
FROM
  events
...
VALUES
  (
    ...
    sqlc.arg(event_date)::timestamp,
    sqlc.arg(status)::text,
    NULLIF(sqlc.arg(home_score)::int, 0),
    NULLIF(sqlc.arg(away_score)::int, 0),
...
-- name: BulkUpsertEvents :many
WITH input_data AS (
  SELECT
    unnest(sqlc.arg(external_ids)::text[]) as external_id,
    unnest(sqlc.arg(league_ids)::int[]) as league_id,
    unnest(sqlc.arg(home_team_ids)::int[]) as home_team_id,
    unnest(sqlc.arg(away_team_ids)::int[]) as away_team_id,
    unnest(sqlc.arg(event_dates)::timestamp[]) as event_date,
    unnest(sqlc.arg(statuses)::text[]) as status,
    unnest(sqlc.arg(bulletin_ids)::bigint[]) as bulletin_id,
    unnest(sqlc.arg(versions)::bigint[]) as version,
    unnest(sqlc.arg(sport_ids)::int[]) as sport_id,
    unnest(sqlc.arg(bet_programs)::int[]) as bet_program,
    unnest(sqlc.arg(mbcs)::int[]) as mbc,
    unnest(sqlc.arg(has_king_odds)::boolean[]) as has_king_odd,
    unnest(sqlc.arg(odds_counts)::int[]) as odds_count,
    unnest(sqlc.arg(has_combines)::boolean[]) as has_combine,
