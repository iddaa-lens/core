...
WHERE 
    (ABS(oh.change_percentage) >= sqlc.arg(min_change_pct) OR oh.multiplier >= sqlc.arg(min_multiplier))
...
WHERE 
    oh.recorded_at >= sqlc.arg(since_time)
    AND e.event_date > NOW()
    AND od.bet_percentage > od.implied_probability + sqlc.arg(min_bias_pct)
    AND ABS(oh.change_percentage) >= sqlc.arg(min_movement_pct)
...
WHERE 
    ...
    AND (sqlc.arg(alert_type) = '' OR ma.alert_type = sqlc.arg(alert_type))
    AND (sqlc.arg(min_severity) = '' OR 
         (ma.severity = 'critical' AND sqlc.arg(min_severity) IN ('low', 'medium', 'high', 'critical')) OR
         (ma.severity = 'high' AND sqlc.arg(min_severity) IN ('low', 'medium', 'high')) OR
         (ma.severity = 'medium' AND sqlc.arg(min_severity) IN ('low', 'medium')) OR
         (ma.severity = 'low' AND sqlc.arg(min_severity) = 'low'))
ORDER BY 
    CASE ma.severity 
